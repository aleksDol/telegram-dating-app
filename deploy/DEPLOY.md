...
**Если при сборке появляется ошибка `429 Too Many Requests` / `toomanyrequests`:** Docker Hub ограничивает число подтягиваний образов без авторизации. Войдите в аккаунт (бесплатный на [hub.docker.com](https://hub.docker.com)) и выполните один раз:

```bash
docker login
```

После входа лимит выше, и сборка обычно проходит. Альтернатива — подождать несколько часов (лимит сбрасывается).

Если у вас есть зеркало Docker (например, корпоративное), можно переопределить базовые образы при сборке:

```bash
docker compose build --build-arg NODE_IMAGE=ваше-зеркало/node:20-alpine --build-arg NGINX_IMAGE=ваше-зеркало/nginx:alpine --build-arg PYTHON_IMAGE=ваше-зеркало/python:3.12-slim
```
